...
static ssize_t gec6818_watch_dog_write(struct file *folp, const char __user *buf, size_t len, loff_t *off)
{
    unsigned char control;
    if(len != 1)
        return -EINVAL;
    if(copy_from_user(&control,buf,len))
        return -EINVAL;


    // 控制是否要复位---------->
    if(control)
        *WTCNT = 0x9896; // 重新写入数据防止复位
    else{
        *WTCON &= (~1);; // 关闭复位信号
    }
	return len;	
...
static int __init gec6818_watch_dog_init(void)
{
	...
	// 创建device
	gec6818_watch_dog_device = device_create(gec6818_watch_dog_class, NULL,watch_dog_dev_num, NULL, "watch_dog_dev");  // /dev/watch_dog_dev
    ...
    printk(KERN_INFO "gec6818 watch_dog driver init8...\n");
    if(wt_res == NULL)
    {
        printk(KERN_INFO "gec6818 watch_dog driver init7...\n");
